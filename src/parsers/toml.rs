...
pub fn parse(
    ...
    for (key, value) in config {
        match (key.as_str(), value) {
            ("game", _) => {},
            ("fps", Value::Integer(v)) => {
                send(sender, ZpMsg::SetFps(v as u64));
            },
            ("flickFactor", Value::Integer(v)) => {
                send(sender, ZpMsg::SetFlickFactor(v as f64));
            },
            ("flickFactor", Value::Float(v)) => {
                send(sender, ZpMsg::SetFlickFactor(v));
            },
            ("flickTime", Value::Integer(v)) => {
                send(sender, ZpMsg::SetFlickTime(v as u64));
            },
            ("flickDeadzone", Value::Float(v)) => {
                send(sender, ZpMsg::SetFlickDeadzone(v as f64));
            },
            ("mapping", Table(mappings)) => {
                parse_mappings(sender, Table(mappings));
            },
            ("layers", Table(layermaps)) => {
                parse_layers(sender, Table(layermaps));
            },
            (key, value) => {
